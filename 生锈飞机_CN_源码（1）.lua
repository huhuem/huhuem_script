...
-- ======================================================
--  云端版本检测（双源：完整源码 + rustyplane_ver.lua）
-- ======================================================
local LOCAL_VERSION = "v0.1 Beta";
local UPDATE_URL = "https://raw.githubusercontent.com/huhuem/huhuem_script/refs/heads/main/%E7%94%9F%E9%94%88%E9%A3%9E%E6%9C%BA_CN_%E6%BA%90%E7%A0%81.lua";
local VER_URL = "https://raw.githubusercontent.com/huhuem/huhuem_script/refs/heads/main/rustyplane_ver.lua";

local function showUpdateNotification(cloudVer)
...
local function checkCloudUpdate()
	...
	if ok and raw then
		local cloudVer = raw:match('LOCAL_VERSION%s*=%s*"([^"]+)"');
		if cloudVer and cloudVer ~= LOCAL_VERSION then
			showUpdateNotification(cloudVer);
			return;
		end
	end

	-- 源2：从 rustyplane_ver.lua 拉取版本号比对
	local ok2, verRaw = pcall(function()
		return game:HttpGet(VER_URL);
	end);
	if ok2 and verRaw then
		-- 支持多种格式：return "v0.2" / "v0.2" / return 'v0.2' / return v0.2
		local verCloudVer = verRaw:match('"([^"]+)"')
			or verRaw:match("'([^']+)'")
			or verRaw:match("return%s+(%S+)");
		if verCloudVer and verCloudVer ~= LOCAL_VERSION then
			showUpdateNotification(verCloudVer);
		end
	end
end
